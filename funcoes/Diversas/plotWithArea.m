function [] = plotWithArea(time,mean,std,axisVector,input,xLabel,imageNameFile)
    close all;
    hold on;
...
%      plot(time, mean(:,1), 'LineWidth', 2, 'Color', [0.95 0.95 0]);
%     plot(time, mean(:,2), 'LineWidth', 2, 'Color', [0.5 0.6 0.9]);
    
    plot(time,input,'-.', 'LineWidth', 2, 'Color', [0.20 0.43 0.19]);
    ...
    ax.YLabel.String = xLabel; %Name of Y axes in Latex format
