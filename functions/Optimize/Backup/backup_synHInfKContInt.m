...
%   Calculates T
   for i=1:nw
       for j=1:nx
            gi = generatesEi(i,nw); 
            productOfDeltaB1Beta1ij = (abs(deltaB1(j,i))^2)*betas1(j,i)*gi*gi';
            
%             First time i = j = 1
            if (i==1 && j == 1) 
                T = productOfDeltaB1Beta1ij;
            else
                T = productOfDeltaB1Beta1ij + T;
            end
...
%   First U sum
    for i=1:ny
       for j=1:nx       
            hi = generatesEi(i,ny); 
            productOfDeltaCGammaij = (abs(deltaC(i,j))^2)*gammas(i,j)*hi*hi';
...
%  Second U sum
   for i=1:ny
       for j=1:nu
            ...
            hi = generatesEi(i,ny); 
            productOfDeltaDEpsilon2ij = (abs(deltaD2(i,j))^2)*epsilons2(i,j)*hi*hi';
            
%             First time i = j = 1
            if (i==1 && j == 1) 
                secondSum = productOfDeltaDEpsilon2ij;
            else
                secondSum = productOfDeltaDEpsilon2ij + secondSum;
            end
   ...
   for i=1:ny
       for j=1:nw
            ...
            hi = generatesEi(i,ny); 
            productOfDeltaDEpsilon1ij = (abs(deltaD1(i,j))^2)*epsilons1(i,j)*hi*hi';
            
%             First time i = j = 1
            if (i==1 && j == 1) 
                thirdSum = productOfDeltaDEpsilon1ij;
            else
                thirdSum = productOfDeltaDEpsilon1ij + thirdSum;
            end
