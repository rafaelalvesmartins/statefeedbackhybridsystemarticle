function outPut = synHInfKContIntWOD1Modified(A,B2,B1,C,D2,D1,param)
...
%             First time i = j = 1
            if (i==1 && j == 1) 
                T = productOfDeltaBBetaij;
            else
                T = productOfDeltaBBetaij + T;
            end
...
%  Second U' sum
   for i=1:ny
       for j=1:nu
            ...
            gi = generatesEi(i,ny); 
            productOfDeltaDEpsilonij = (abs(deltaD2(i,j))^2)*epsilons2(i,j)*gi*gi';
...
%   Xa, Lb2 and I1
    ...
    Epsilons = diag(reshape(epsilons2',[nu*ny, 1]));
