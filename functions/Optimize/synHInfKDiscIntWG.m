function outPut = synHInfKDiscIntWG(A,B2,B1,C,D2,D1,param)
    % Calculates hinf norm based on lema 31 from Ricardo's slides 

   if nargin == 7
        if isfield(param,'tol')
            tol = param.tol;
        else
            tol = 1e-7;
        end    
    else
        tol = 1e-7;
   end
    %    ---------------------Usefull matrices----------------------------
    A0 = mid(A);
    deltaA = rad(A);
    
    B20 = mid(B2);
    deltaB2 = rad(B2);

    B10 = mid(B1);
    deltaB1 = rad(B1);

    C0 = mid(C);
    deltaC = rad(C);

    D20 = mid(D2);
    deltaD2 = rad(D2);
    
    D10 = mid(D1);
    deltaD1 = rad(D1);
    
    
%    ---------------------Dimension of input----------------------------
    nx = length(A.inf);
    nu = size(B2.inf,2);
    nw = size(B1.inf,2);
    ny = size(C.inf,1);
    
%    ---------------------Variable Declariation----------------------------
    outPut.cpusec_m = clock;
    %LMI rows counter
    outPut.line = 0;
    
    %   New LMI system
    LMIs = set([]);

    L = sdpvar(nu,nx,'full');
    G = sdpvar(nx,nx,'full');
    X = sdpvar(nx,nx,'symmetric');
    LMIs = LMIs + (X >= 0);
    outPut.line = outPut.line + nx;
  
       %   Set obj  
        mu = sdpvar(1,1);
        obj = mu;
   
     
   %   Alphas 
    for i=1:nx
        for j=1:nx
            alphas(i,j) = sdpvar(1,1); % Scalar
        end
    end
    
 %    Betas 2
    for i=1:nx
        for j=1:nu
            betas2(i,j) = sdpvar(1,1); % Scalar
        end
    end
    
 %    Betas 1
    for i=1:nx
        for j=1:nw
            betas1(i,j) = sdpvar(1,1); % Scalar
        end
    end
    
  %    Gammas
    for i=1:ny
        for j=1:nx
            gammas(i,j) = sdpvar(1,1); % Scalar
        end
    end
    
%    Epsilons 2
    for i=1:ny
        for j=1:nu
            epsilons2(i,j) = sdpvar(1,1); % Scalar
        end
    end
    
%    Epsilons 1
    for i=1:ny
        for j=1:nw
            epsilons1(i,j) = sdpvar(1,1); % Scalar
        end
    end
    
    %   Xa, Lb and Ib1
    for i=1:nx
        if(i==1)
            Ga = G';
			Lb = L';
			Ib1 = eye(nw);
        else
            Ga = [Ga G'];
            Lb = [Lb L'];
            Ib1 = [Ib1 eye(nw)];
        end
    end
    Ga = Ga';
    Lb2 = Lb';
    Ib1 = Ib1';
    
     %   Xc, Ld and Id1
    for i=1:ny
        if(i==1)
            Gc = G';
			Ld = L';
            Id1 = eye(nw);
        else
            Gc = [Gc G'];
			Ld = [Ld L'];
            Id1 = [Id1 eye(nw)];
        end
    end
    Gc = Gc';
    Ld2 = Ld';
    Id1 = Id1';
    
    %   Calculates S
%   First S' sum
    for i=1:nx
       for j=1:nx       
            ei = generatesEi(i,nx); 
            productOfDeltaAAlphaij = (abs(deltaA(i,j))^2)*alphas(i,j)*(ei*ei');
            
%             First time i = j = 1
            if (i==1 && j == 1) 
                firstSumS = productOfDeltaAAlphaij;
            else
                firstSumS = productOfDeltaAAlphaij + firstSumS;
            end
            
       end
    end
    
    %   First S' sum
    for i=1:nx
       for j=1:nu       
            ei = generatesEi(i,nx); 
            productOfDeltaB2Betas2ij = (abs(deltaB2(i,j))^2)*betas2(i,j)*(ei*ei');
            
%             First time i = j = 1
            if (i==1 && j == 1) 
                secondSumS = productOfDeltaB2Betas2ij;
            else
                secondSumS = productOfDeltaB2Betas2ij + secondSumS;
            end
            
       end
    end
    
    %   Third S' sum
    for i=1:nx
       for j=1:nw       
            ei = generatesEi(i,nx); 
            productOfDeltaB1Betas1ij = (abs(deltaB1(i,j))^2)*betas1(i,j)*(ei*ei');
            
%             First time i = j = 1
            if (i==1 && j == 1) 
                thirdSumS = productOfDeltaB1Betas1ij;
            else
                thirdSumS = productOfDeltaB1Betas1ij + thirdSumS;
            end
            
       end
    end
    
    S = firstSumS + secondSumS + thirdSumS;
    
    
    %   Calculates T
%   First T' sum
    for i=1:ny
       for j=1:nx       
            gi = generatesEi(i,ny); 
            productOfDeltaCGammaij = (abs(deltaC(i,j))^2)*gammas(i,j)*(gi*gi');
            
%             First time i = j = 1
            if (i==1 && j == 1) 
                firstSumT = productOfDeltaCGammaij;
            else
                firstSumT = productOfDeltaCGammaij + firstSumT;
            end
            
       end
    end
    
%   Second T' sum
    for i=1:ny
       for j=1:nu       
            gi = generatesEi(i,ny); 
            productOfDeltaD2Epsilons2ij = (abs(deltaD2(i,j))^2)*epsilons2(i,j)*(gi*gi');
            
%             First time i = j = 1
            if (i==1 && j == 1) 
                secondSumT = productOfDeltaD2Epsilons2ij;
            else
                secondSumT = productOfDeltaD2Epsilons2ij + secondSumT;
            end
            
       end
    end
    
    
    %   Third T' sum
    for i=1:ny
       for j=1:nw       
            gi = generatesEi(i,ny); 
            productOfDeltaD1Epsilons1ij = (abs(deltaD1(i,j))^2)*epsilons1(i,j)*(gi*gi');
            
%             First time i = j = 1
            if (i==1 && j == 1) 
                thirdSumT = productOfDeltaD1Epsilons1ij;
            else
                thirdSumT = productOfDeltaD1Epsilons1ij + thirdSumT;
            end
            
       end
    end
    
    T = firstSumT + secondSumT + thirdSumT;
    
    Alphas = diag(reshape(alphas',[nx*nx, 1]));
    Gammas = diag(reshape(gammas',[nx*ny, 1]));
    Betas2 = diag(reshape(betas2',[nu*nx, 1]));
    Betas1 = diag(reshape(betas1',[nw*nx, 1]));
    Epsilons2 = diag(reshape(epsilons2',[nu*ny, 1]));
    Epsilons1 = diag(reshape(epsilons1',[nw*ny, 1]));
    
%     First column 
    U11 = X-S;
    U21 = G'*A0'+L'*B20';
    U31 = zeros(size(C0*G,1),size(U21,2));
    U41 = B10';
    U51 = zeros(size(Ga,1),size(U41,2));
    U61 = zeros(size(Gc,1),size(U41,2));
    U71 = zeros(size(Lb2,1),size(U41,2));
    U81 = zeros(size(Ld2,1),size(U41,2));
    U91 = zeros(size(Ib1,1),size(U41,2));
    U101 = zeros(size(Id1,1),size(U41,2));
    
%     Second Column
    U22 = G+G'-X;
    U32 = C0*G+D20*L;
    U42 = zeros(size(U41,1),size(U22,2));
    U52 = Ga;
    U62 = Gc;
    U72 = Lb2;
    U82 = Ld2;
    U92 =zeros(size(U91,1),size(U22,2));
    U102 = zeros(size(U101,1),size(U22,2));
    
%     Third Column
    U33 = eye(size(T,1))-T;
    U43 = D10';
    U53 = zeros(size(U51,1),size(U33,2));
    U63 = zeros(size(U61,1),size(U33,2));
    U73 = zeros(size(U71,1),size(U33,2));
    U83 = zeros(size(U81,1),size(U33,2));
    U93 = zeros(size(U91,1),size(U33,2));
    U103 = zeros(size(U101,1),size(U33,2));
    
%     Forth Column
    U44 = mu*eye(size(D10',1));
    U54 = zeros(size(U51,1),size(U44,2));
    U64 = zeros(size(U61,1),size(U44,2));
    U74 = zeros(size(U71,1),size(U44,2));
    U84 = zeros(size(U81,1),size(U44,2));
    U94 = Ib1;
    U104 = Id1;
    
%    Fivth Column
    U55 = Alphas;
    U65 = zeros(size(U61,1),size(U55,2));
    U75 = zeros(size(U71,1),size(U55,2));
    U85 = zeros(size(U81,1),size(U55,2));
    U95 = zeros(size(U91,1),size(U55,2));
    U105 = zeros(size(U101,1),size(U55,2));
    
% Sith Column
    U66 = Gammas;
    U76 = zeros(size(U71,1),size(U66,2));
    U86 = zeros(size(U81,1),size(U66,2));
    U96 = zeros(size(U91,1),size(U66,2));
    U106 = zeros(size(U101,1),size(U66,2));
    
%     Seventh Column
    U77 = Betas2;
    U87 = zeros(size(U81,1),size(U77,2));
    U97 = zeros(size(U91,1),size(U77,2));
    U107 = zeros(size(U101,1),size(U77,2));
    
%     Eigth Column
    U88 = Epsilons2;
    U98 = zeros(size(U91,1),size(U88,2));
    U108 = zeros(size(U101,1),size(U88,2));
    
%     Nivth
    U99 = Betas1;
    U109 = zeros(size(U101,1),size(U99,2));
      
%   Tenth
    U1010 = Epsilons1;
    
    
    
    
    T = [U11    U21'   U31'     U41'    U51'    U61'    U71'    U81'    U91'    U101';
         U21    U22    U32'     U42'    U52'    U62'    U72'    U82'    U92'    U102';
         U31    U32    U33      U43'    U53'    U63'    U73'    U83'    U93'    U103';
         U41    U42    U43      U44     U54'    U64'    U74'    U84'    U94'    U104';
         U51    U52    U53      U54     U55     U65'    U75'    U85'    U95'    U105';
         U61    U62    U63      U64     U65     U66     U76'    U86'    U96'    U106';
         U71    U72    U73      U74     U75     U76     U77     U87'    U97'    U107';
         U81    U82    U83      U84     U85     U86     U87     U88     U98'    U108';
         U91    U92    U93      U94     U95     U96     U97     U98     U99     U109';
         U101   U102   U103     U104    U105    U106    U107    U108    U109    U1010];
    
    LMIs = LMIs + (T >= 0);

    outPut.line = outPut.line + size(T,1);
       
    
   
    outPut.cpusec_m = etime(clock,outPut.cpusec_m);
    outPut.var = size(getvariables(LMIs),2);
    
    sol = optimize(LMIs,obj,sdpsettings('shift','1','verbose',0,'solver','mosek','sedumi.eps',1e-20,'sedumi.maxiter',2000,'sedumi.numtol',1e-20));
%     sol = optimize(LMIs,obj,sdpsettings('verbose',0,'solver','sedumi'));
    
    
    outPut.cpusec = sol.solvertime;
    p = min(checkset(LMIs));
    outPut.delta = p;

    if p > -tol
        outPut.feas = 1;
        
       
%          if(~(nargin == 7)||(nargin == 7 && ~isfield(param,'mu')))
%            param.mu = double(mu)+episilonHinf;
%            synPoly = synHInfKContInt(A,B2,B1,C,D2,D1,param);
%            outPut.mu = sqrt(synPoly.mu);
%            outPut.L =  synPoly.L;
%            outPut.X = synPoly.X;
%            outPut.K = synPoly.K;
%            
%            outPut.withoutExtLMI.L = double(L);
%            outPut.withoutExtLMI.X =  double(X);
%            outPut.withoutExtLMI.K = outPut.withoutExtLMI.L*outPut.withoutExtLMI.X^-1;
%            outPut.withoutExtLMI.mu = sqrt(double(mu));
%           
%            
%            outPut.withoutExtLMI.K =  outPut.withoutExtLMI.L*outPut.withoutExtLMI.X^-1;
%            
%            
%         else
           outPut.L = double(L);
           outPut.G =  double(G);
           outPut.K = outPut.L*outPut.G^-1;
           outPut.norm = sqrt(double(mu)); 
%         end
        
        
        
    else
        outPut.feas = 0;        
    end
end